...
//		Predict_eleangle = fmodf(Predict_eleangle, Two_PI);
    ...
    get_weight_from_phasecurrent(foc_abc_current_i.ia, foc_abc_current_i.ib, foc_abc_current_i.ic);
	 
    Vbus = ADC_ConvertedValue[0]* V_SCALE; // ADC1->JDR2 * V_SCALE;  
		/*********************  Processing EventTask   *************************/
//	   static int flag = 0, Resistance_Identify_flag = 0, Inductance_Identify_flag = 0;;
////	   if(flag == 0)
////	   {
////         Resistance_Identify_flag = Resistance_Identify_Loop(Vbus, PWM_PERIOD, 0, &Phase_Ra); 
////			   if(Resistance_Identify_flag == 1)
////					 flag++;
////	   }
////		 else if(flag == 1){
////			   Resistance_Identify_flag = Resistance_Identify_Loop( Vbus, PWM_PERIOD, 2*PI/3.0f,&Phase_Rb);
////			   if(Resistance_Identify_flag == 1)
////					 flag++;
////		 }
////		 else if(flag == 2){
////			   Resistance_Identify_flag = Resistance_Identify_Loop(Vbus, PWM_PERIOD, 4*PI/3.0f, &Phase_Rc);
////			   if(Resistance_Identify_flag == 1)
////					 flag++;
////		 }
//		 
//		 if(flag == 0){
//		    //Inductance_Identify_flag = Inductance_Identify_Loop(Vbus, PWM_PERIOD, timFactor, 0.0f, Phase_Ra, &Phase_La);
//			 
//			 
//        Inductance_Identify_flag = Inductance_Identify_HF_Loop(Vbus, PWM_PERIOD, timFactor,  0.0f, 
//																		2.23f, 976.5625, 4.0f, &Phase_La, &Phase_Ra);  //云台电机   976.5625/1953.125f - 512 - 50K    976.5625/1953.125f - 512 - 10K
////				  Inductance_Identify_flag = Inductance_Identify_HF_Loop(Vbus, PWM_PERIOD, timFactor,  0.0f, 
////																			0.5f, 976.5625, 4.0f, &Phase_La);  //HALL电机  
//			   if(Inductance_Identify_flag == 1)
//					  flag++;
//		 }
//		 else if(flag == 1){
//		    //Inductance_Identify_flag = Inductance_Identify_Loop(Vbus, PWM_PERIOD, timFactor, 2*PI/3.0f, Phase_Rb, &Phase_Lb);
//			  Inductance_Identify_flag = Inductance_Identify_HF_Loop(Vbus, PWM_PERIOD, timFactor,  90.0f, 
//																		2.23f, 976.5625, 4.0f, &Phase_Lb, &Phase_Rb);  //云台电机   976.5625/1953.125f - 512 - 50K    976.5625/1953.125f - 512 - 10K
////				  Inductance_Identify_flag = Inductance_Identify_HF_Loop(Vbus, PWM_PERIOD, timFactor,  90.0f, 
////																			0.5f, 976.5625, 4.0f, &Phase_La);  //HALL电机  
//			 
//			   if(Inductance_Identify_flag == 1)
...
//		 }
////		else if(flag == 2){
////		    Inductance_Identify_flag = Inductance_Identify_Loop(Vbus, PWM_PERIOD, timFactor, 4*PI/3.0f, Phase_Rc, &Phase_Lc);
////			   if(Inductance_Identify_flag == 1)
////					  flag++;
////		 }
